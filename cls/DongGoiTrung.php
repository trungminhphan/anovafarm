...
class DongGoiTrung{
	...
	public function __construct(){
		...
		$this->_collection = $this->_mongo->donggoitrung;
	}
	...
	public function insert(){
		$query = array(
			'id_nongtraitrung' => DBConnect::ObjectId($this->id_nongtraitrung),
	...
	public function check_nhamay($id_nhamay){
		$query = array('id_nhamay' => DBConnect::ObjectId($id_nhamay));
	...
	public function check_nongtrai($id_nongtraitrung){
		$query = array('id_nongtraitrung' => DBConnect::ObjectId($id_nongtraitrung));
		$field = array('_id' => true);
		$result = $this->_collection->findOne($query, $field);
		if(isset($result['_id']) && $result['_id']) return true;
		else return false;
	...
	public function get_one_by_nongtrai(){
		$query = array('id_nongtraitrung' => DBConnect::ObjectId($this->id_nongtraitrung));
		return $this->_collection->findOne($query);
	}
